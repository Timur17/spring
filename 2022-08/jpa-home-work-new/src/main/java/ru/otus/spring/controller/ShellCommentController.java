...
public class ShellCommentController {
    ...
    public void insert(String comment, String bookId) {
        Comment commentEntity = commentService.insert(comment, bookId);
        if (commentEntity != null) {
            consoleIOService.outputString("Comment was added to bookId: " + commentEntity.getBookId());
        } else {
    ...
    public void ShowAllCommentsByBookId(String id) {
        ...
        if (comments.size() == 0) {
            consoleIOService.outputString("Book not exist with id: " + id);
        } else {
            comments.forEach(comment -> {
                consoleIOService.outputString("Comment: " + comment);
            });
        }
